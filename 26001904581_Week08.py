...
def randomGene():
    rand_gene = random.choice(string.printable)
    return rand_gene
...
def Fit_func1(competing_DNA):
    ...
    for i in range(dnalength):
        compete_gene = ord(competing_DNA[i])
        target_gene = ord(target[i])# the fitness value is the absolute value of difference between the target DNA and the competing DNA at this position in both DNAs
        fitness = abs(compete_gene - target_gene)
        total += fitness # final fitness value = 1 divided by total sum of all fitness values
    return total #return output of the function, fitness

def Fit_func2(competing_DNA):
    total = 0
    for i in range(dnalength):
        if (ord(competing_DNA[i]) - ord(target[i])) != 0:
            ...
            total = 1/(total + fitness) # final fitness value = 1 divided by total sum of all fitness values
    return fitness #return output of the function, fitness

def Fit_func3(competing_DNA):
    fitness = 0
    for i in range(dnalength):
        if ord(competing_DNA[i]) != ord(target[i]):
            fitness = (ord(competing_DNA[i])- ord(target[i]))*1# if the value of character in target dna at this position is different from the value of character in competing dna at this position, for every time there's a difference, 1 is added
            total = 1 /(total + fitness) # final fitness value = 1 divided by total sum of all fitness values
    return fitness 
...
def mutation(competing_DNA, mutation_ratio):
    ...
    DNA_index = random.randint(0, len(competing_DNA)) #pick a random integer that falls within the length of the competing DNA
    ...
    if mutate_prob == 1: # set up the mutation process that has a 1/100 chance to happen
        competing_DNA[DNA_index] = random_gene # assign the randomly generated gene from random gene function to the index position in this DNA
...
def weightedDNAchoice(competeDNAfitpair):
    ...
    probs /= probs.sum()
    return competeDNAfitpair [np.random.choice(len(competeDNAfitpair), 1, p = probs)[0]][0] #return output of the function. normalizes values and put them in an array of probabilities

currentpop = inipopulation()
for i in range(generation):
    lastfitarray = []
    for k in currentpop:
        lastfitarray.append(Fit_func1(k)) # print generation number, and the fittest string for the current generation
...
for individual in currentpop:
    indipenalty = Fit_func1(individual) # use the value "individual" for fitness function
    if indipenalty == 0:
...
lastfitarray = []
for g in currentpop:
    lastfitarray.append(Fit_func1(g))
