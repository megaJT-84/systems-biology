...
def errorFunc3(x):
    sum_func3 = sum([(15 * m.sin((x[i]/5)**2)) * (75 * m.cos(x[i]/13)) * (3 * m.sin(x[i]/6)) + x[i]**2 + 300 for i in range(len(x))])
    return sum_func3
...
# update the particle velocity
    def update_vel(self, groupBestPosition):
        w = 5 # inertia weight of previous weight
...
particleSwarmOP(errorFunc3, initialStartingPosition, minMaxBounds, num_particles = 150, maxiter = 150)
